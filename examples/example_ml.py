...
# Run ML model:
ts_pred = ml_stats.predict_ts(ts_origin=df_nora3.data,var_origin=var_origin,ts_train=df_norac.data.loc[start_training:end_training],var_train=['hs'], model=model)
...
plt.scatter(df_norac.data['hs'], ts_pred.loc['2018-01-01':'2019-12-31'], color='black')
plt.title('scatter:'+model+'-'+'_'.join(var_origin))
plt.text(0, 2,'ΜΑΕ:'+str(sklearn.metrics.mean_absolute_error(df_norac.data['hs'].loc[start_valid:end_valid], ts_pred.loc[start_valid:end_valid]).round(3)))
plt.text(0, 1.8,'$R^2$:'+str(sklearn.metrics.r2_score(df_norac.data['hs'].loc[start_valid:end_valid], ts_pred.loc[start_valid:end_valid]).round(3)))
plt.text(0, 1.6,'RMSE:'+str((sklearn.metrics.mean_squared_error(df_norac.data['hs'].loc[start_valid:end_valid], ts_pred.loc[start_valid:end_valid])**0.5).round(3)))
