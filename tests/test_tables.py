...
def test_table_binned_error_metric(ds=ds):
    ...
    df = tables.table_binned_error_metric(data=df,var_bin = 'HS', var_ref='TP',var_comp='TP', var_bin_size=1,threshold_min=100, error_metric=['rmse','bias','mae','corr','si'],output_file=output_file)
    if os.path.exists(output_file):
        os.remove(output_file)
    if df.shape == (9, 8):
        pass
