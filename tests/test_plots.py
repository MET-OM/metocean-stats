...
def test_plot_monthly_weather_window(ds=ds):
    ...
    fig, table = plots.plot_monthly_weather_window(ds, var='HS', threshold=4, timestep=3, window_size=12, output_file=output_file)
    if os.path.exists(output_file):
        os.remove(output_file)
    if table._loc == 17:
        pass
...
def test_plot_monthly_weather_window_mv(ds=ds):
