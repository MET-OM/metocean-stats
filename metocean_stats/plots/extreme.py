...
def plot_monthly_return_periods(data, var='hs', periods=[1, 10, 100, 10000],distribution='Weibull',negative=False, units='m',output_file='monthly_extremes_weibull.png'):
    df = table_monthly_return_periods(data=data,var=var, periods=periods,distribution=distribution,negative=negative, units=units, output_file=None)
    fig, ax = plt.subplots()
    for i in range(len(periods)):
