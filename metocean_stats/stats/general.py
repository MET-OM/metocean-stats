...
def table_current_depth_stat(file='data.pkl'):
    ...
    df = pd.DataFrame()
    df['depth'] = layers
    df['mean'] = np.mean(speed,axis=0)
    df['std'] = np.std(speed,axis=0)
    df['P5'] = np.percentile(speed,5,axis=0)
    df['P10'] = np.percentile(speed,10,axis=0)
    df['P50'] = np.percentile(speed,50,axis=0)
    df['P90'] = np.percentile(speed,90,axis=0)
    df['P95'] = np.percentile(speed,95,axis=0)
    df['P99'] = np.percentile(speed,99,axis=0)
    df['Max'] = np.max(speed,axis=0)
    
    index_max = np.argmax(speed,axis=0)
    df['DirMaxSpeed'] = [direction[index_max[i],i] for i in range(len(index_max))]
    df['TimeMaxSpeed'] = [time[i] for i in index_max]
    
    df = df.iloc[::-1]
    df.to_csv('basic_statistics_all_depths.csv', index=False)  

    return df 

def table_Weibul_RVE(file='data.pkl'):

    def RVE_Weibull(df,var,period=100):
    
        shape, loc, scale = Weibull_method_of_moment(df[var].values)
    ...
    period=np.array([1,10,100,10000])
    rve = np.zeros((speed.shape[1], period.shape[0]))
    ...
    for i in range(speed.shape[1]):
        ...
        rve[i,:] = RVE_Weibull(df,'speed',period=period)
    ...
    df['10'] = df['10'].astype(float).round(2)
    df['100'] = df['100'].astype(float).round(2)
    df['10000'] = df['10000'].astype(float).round(2)
    
    df.to_csv('RVE.all.depths.csv', index=False)  
    
    return df


def plt_depth_profiles():
    
    df2 = pd.read_csv('basic_statistics_all_depths.csv')

    df = pd.read_csv('RVE.all.depths.csv')

    plt.plot(df2['P5'].values,df2.depth.values,'--',label='P5')
    plt.plot(df2['P10'].values,df2.depth.values,'--',label='P10')
    plt.plot(df2['P50'].values,df2.depth.values,'--',label='P50')
    plt.plot(df2['P90'].values,df2.depth.values,'--',label='P90')
    plt.plot(df2['P95'].values,df2.depth.values,'--',label='P95')
    plt.plot(df2['P99'].values,df2.depth.values,'--',label='P99')
    plt.plot(df2['Max'].values,df2.depth.values,'--',label='Max')
    plt.plot(df['1'].values,df.depth.values,'k',label='1 year')
    plt.plot(df['10'].values,df.depth.values,'c',label='10 year')
    plt.plot(df['100'].values,df.depth.values,label='100 year')
    plt.plot(df['10000'].values,df.depth.values,'r',label='10000 year')
    plt.xlim(0,2)
    plt.grid()
