...
def table_binned_error_metric(dff, var_bin, var_ref, var_comp, var_bin_size, threshold_min, error_metric=['scatter_index','rmse','bias','mae','corr','si'],output_file='binned_error_metric.csv'):
    """
    ...
    for i in range(len(dff)-1):
        noise = np.random.uniform(0, 1, size=len(dff[0].TP))

        df_ref = dff[0].dropna()
        df_comp = dff[i+1].dropna()
        max_var3 = max(dff[i][var_bin]) 
        window = np.arange(0, max_var3 + var_bin_size, var_bin_size) #the max should be the max of the shortest comp.
        var_xx = []
        var_yy1 = []
        var_comp1 = var_comp
        ...
        for j in range(len(window)-1):
            #var_3 = df_comp[var_bin].where((df_comp[var_bin] > window[j]) & (df_comp[var_bin] <= window[j+1])).dropna() #windo = (windo1,windo2]
            var_3 = df_ref[var_bin].where((df_ref[var_bin] > window[j]) & (df_ref[var_bin] <= window[j+1])).dropna() #windo = (windo1,windo2]
            if len(var_3)<threshold_min:
                continue
            var_xx = df_ref[var_comp1].where(var_3.notnull()).dropna() 
            var_yy1 = ((df_comp[var_comp1]).where(var_3.notnull())+noise).dropna() 
            ...
            dss_temp = pd.DataFrame({var_comp+'vs'+var_ref: ['-'] , var_bin+'_bin':[var_bin_mean], var_comp1:[var_yy_mean]}) #temp dataframe
